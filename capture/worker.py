...
class CaptureWorker(QThread):
    ...
    def open_camera(self, index: int, size: tuple[int, int] | None = None) -> None:
    ...
    def set_resolution(self, size: tuple[int, int] | None) -> None:
        """Request a new capture resolution (None = camera default)."""
        with QMutexLocker(self._mutex):
            self._desired_size = size
            self._reopen_requested = True
    ...
    def run(self) -> None:
        while True:
            ...
            try:
                self._ensure_capture_open(index, desired)
            ...
            if not ok or frame is None:
                ...
                if self._consecutive_failures >= 30:
                    self.error_occurred.emit("Camera disconnected or stopped delivering frames.")
                    with QMutexLocker(self._mutex):
                        self._stop_recording_locked()
                        self._close_capture_locked()
                    self._consecutive_failures = 0
    ...
    def _ensure_capture_open(self, index: int, desired: tuple[int, int] | None) -> None:
        if self._cap is not None and self._cap.isOpened():
            return
        self._close_capture_locked()
        cap = open_capture(index)
        # Request MJPG first: most UVC/HDMI capture dongles can only deliver
        # full resolution at 30 FPS in MJPG. The raw (YUYV) default exceeds USB
        # bandwidth at 1080p, so the device silently drops to a few FPS and the
        # preview stutters even though CAP_PROP_FPS still reports 30.
        ...
        try:
            # Keep latency low so the preview shows the most recent frame.
        ...
        if not fps or fps <= 1:
            fps = 30.0
        self._cap = cap
        self.camera_opened.emit(index, float(fps), width, height)
